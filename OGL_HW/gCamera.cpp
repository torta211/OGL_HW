...
{
	// simply not let the camera get into negative y
	glm::vec3 delta_forth = m_goFw * m_fw * m_speed * _deltaTime;;
	glm::vec3 delta_sides = m_goRight * m_st * m_speed * _deltaTime;
	if ((m_eye + delta_forth)[1] > 0.01)
	{
		m_eye += delta_forth;
		m_at += delta_forth;
	}
	if ((m_eye + delta_sides)[1] > 0.01)
	{
		m_eye += delta_sides;
		m_at += delta_sides;
	}
